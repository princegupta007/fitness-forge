                                ...
                                <div className="flex-1 bg-surface border-x border-b border-white/5 p-4 space-y-4">
                                    {groupedSchedule[day]?.length > 0 ? (
                                        groupedSchedule[day].map((entry) => (
                                            <div key={entry._id} className="p-4 bg-black/40 border border-white/5 hover:border-accent/40 transition-colors group">
                                                <div className="flex items-center gap-2 text-accent mb-2">
                                                    <Clock size={14} />
                                                    <span className="text-[10px] font-black uppercase tracking-tighter">{entry.time}</span>
                                                </div>
                                                ...
                                                <div className="flex items-center gap-2 text-slate-500">
                                                    <User size={12} />
                                                    <span className="text-[10px] font-bold uppercase tracking-widest">{entry.trainer}</span>
                                                </div>
                                            </div>
                                        ))
                                    ) : (
                ...
                <Container className="text-center">
                    <div className="max-w-2xl mx-auto p-8 border border-white/10 bg-black/40">
                        ...
                        <p className="text-slate-400 mb-8">
                            All classes require 24-hour advance booking through the member app or at the front desk.
                            Trial members please contact us to reserve your spot.
                        </p>
